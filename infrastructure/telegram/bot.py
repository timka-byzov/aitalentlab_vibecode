...
class AdmissionBot:
    ...
    async def program_selected(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        if user_choice not in PROGRAM_MAP:
            if update.effective_message:
                await update.effective_message.reply_text("Пожалуйста, выберите программу, используя предложенные кнопки.")
            return PROGRAM
    ...
    async def background_received(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        try:
            if not update.effective_message or not update.effective_message.text:
                return ConversationHandler.END
            ...
            background = {
                "math": scores[0], "programming": scores[1], "ai": scores[2],
                "data": scores[3], "product": scores[4]
            }
            ...
            if not recommended:
                if update.effective_message:
                    await update.effective_message.reply_text("Не удалось подобрать курсы по вашему бэкграунду.")
                return ConversationHandler.END
            ...
            if update.effective_user:
                logging.error(f"Ошибка при генерации учебного плана для user_id {update.effective_user.id}:", exc_info=True)
            if update.effective_message:
                await update.effective_message.reply_text(
    ...
    async def answer_question(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        if "program_id" not in context.chat_data:
            if update.effective_message:
                await update.effective_message.reply_text("Программа не выбрана. Пожалуйста, начните заново с /start")
            return ConversationHandler.END
        ...
        if not program_id:
            if update.effective_message:
                await update.effective_message.reply_text("Что-то пошло не так. Пожалуйста, начни заново с /start")
            return ConversationHandler.END

        program = self.curricula.get(program_id)
        if not program:
            if update.effective_message:
                await update.effective_message.reply_text("Программа не найдена. Пожалуйста, начните заново с /start")
            return ConversationHandler.END

        if "обязательные" in question and "семестр" in question:
            semester = 1 if "первый" in question or "1" in question else 2 if "второй" in question or "2" in question else None
            if semester:
                courses = [c.name for c in program.courses if c.semester == semester and c.is_compulsory]
                response = f"Обязательные курсы в {semester} семестре:\n• " + "\n• ".join(courses) if courses else f"Не найдено обязательных курсов в {semester} семестре."
                if update.effective_message:
...
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            PROGRAM: [MessageHandler(filters.TEXT & ~filters.COMMAND, bot.program_selected)],
            BACKGROUND: [MessageHandler(filters.TEXT & ~filters.COMMAND, bot.background_received)],
            QUESTIONS: [MessageHandler(filters.TEXT & ~filters.COMMAND, bot.answer_question)],
        },
